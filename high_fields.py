...
def load_sheet(tab_name):
    ...
    if not df.empty:
        # Strip stray whitespace from header cells (e.g. "Maintenance " vs
        # "Maintenance") so column lookups elsewhere don't silently fail.
        df.columns = df.columns.astype(str).str.strip()
    return df

def require_columns(df, required, sheet_name):
    """Show a clear, non-crashing error if a sheet is missing expected columns."""
    missing = [c for c in required if c not in df.columns]
    if missing:
        st.error(
            f"The '{sheet_name}' sheet is missing column(s): {', '.join(missing)}. "
            f"Found columns: {', '.join(df.columns) if len(df.columns) else '(none)'}. "
            "Please check the header row for typos or extra spaces."
        )
        st.stop()

def coerce_numeric(df, cols):
    """Force listed columns to numbers (strips $ , etc.), non-numeric cells become 0."""
    for col in cols:
...
def append_row(tab_name, row_dict):
    ...
    # value_input_option="RAW" writes numbers as actual numbers (not text/formulas),
    # so numeric cells don't come back as strings on the next read.
...
if st.session_state.role == "admin":
    ...
    if not records.empty:
        require_columns(records, ["Address", "Rent_Received", "Receiving_Partner",
                                   "Maintenance", "IT_Subscription", "Other_Income",
                                   "Other_Expenses", "Profit", "Rent_Owing", "Month"],
                         "MonthlyRecords")
        records = coerce_numeric(records, ["Rent_Received", "Maintenance", "IT_Subscription",
                                            "Other_Income", "Other_Expenses", "Profit", "Rent_Owing"])
    if not houses_df.empty:
        require_columns(houses_df, ["Address", "Fixed_Rent", "Active"], "Houses")
        houses_df = coerce_numeric(houses_df, ["Fixed_Rent"])
            ...
            if submitted:
                if not house or not receiving_partner:
                    ...
                    new_row = {
                        ...
                        "Rent_Received": float(rent_received),
                        "Receiving_Partner": receiving_partner,
                        "Maintenance": float(maintenance_input),
                        "IT_Subscription": float(it_input),
                        "Other_Income": float(other_income),
                        "Other_Expenses": float(other_expenses),
                        "Profit": float(profit),
                        "Rent_Owing": float(rent_owing),
