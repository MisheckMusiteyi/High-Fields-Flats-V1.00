...
Original file is located at
    https://colab.research.google.com/drive/1K-FSrOmeMRVglJJrn3qX3zhrY5FilNjr
"""
...
def get_active_houses():
    ...
    if not houses.empty:
        active = houses[houses["Active"] == True]
    ...
    with tab2:
        ...
        with st.form("add_record", clear_on_submit=True):
            ...
            if not houses_df.empty:
                house_row = houses_df[houses_df["House_Number"] == house]
    ...
    with tab1:
        ...
        if records.empty:
            ...
            if selected_house != "All":
                filtered = filtered[filtered["House_Number"] == selected_house]
            if len(date_range) == 2:
                start, end = date_range
                filtered = filtered[(filtered["Month"].dt.date >= start) & (filtered["Month"].dt.date <= end)]
